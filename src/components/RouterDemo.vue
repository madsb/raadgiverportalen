<template>
  <div class="router-demo-section">
    ...
    <p>
      Denne sektion demonstrerer den tilpassede hash-baserede router, som er udviklet specifikt til Virksomhedsguiden-miljøet.
      Routeren understøtter dyb linking, browser navigation, route guards og nested layouts - alt sammen uden at bruge
    ...
    <div class="row">
      <div class="col-12">
        <div class="router-info mb-4">
          ...
          <dl class="info-list">
            <dt>Path:</dt>
            <dd><code>{{ router.path || '/' }}</code></dd>
            <dt>Parametre:</dt>
            <dd><code>{{ JSON.stringify(router.params) }}</code></dd>
            <dt>Matched komponent:</dt>
            <dd>{{ router.chain[0]?.record?.component?.name || 'Ingen' }}</dd>
            <dt>Meta data:</dt>
            <dd><code>{{ JSON.stringify(router.chain[0]?.record?.meta || {}) }}</code></dd>
          </dl>
        </div>
        ...
        <div class="router-controls mb-4">
          ...
          <div class="button-group">
            <button class="button button-secondary" @click="navigate('/')">
              Forside
            </button>
            <button class="button button-secondary" @click="navigate('/kunder')">
              Kunder (kræver login)
            </button>
            <button class="button button-secondary" @click="navigate('/kunder/42')">
              Kunde #42
            </button>
            <button class="button button-secondary" @click="navigate('/kunder/42/projekter')">
              Projekter for kunde #42
            </button>
            <button class="button button-secondary" @click="navigate('/kunder/42/projekter/123')">
              Projekt #123
            </button>
            <button class="button button-secondary" @click="navigate('/ikke-fundet')">
              404 Side
            </button>
          </div>
        </div>
        ...
        <div class="scroll-behavior-demo mt-4">
          ...
          <div class="button-group">
            <button class="button button-secondary" @click="setDefaultScrollBehavior">
              Standard (scroll til top)
            </button>
            <button class="button button-secondary" @click="setKeepPositionBehavior">
              Behold position
            </button>
            <button class="button button-secondary" @click="setConditionalBehavior">
              Betinget (kun ved nye ruter)
            </button>
          </div>
          <p class="mt-2">
